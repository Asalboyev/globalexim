...
@section('content')
    ...
    <a href="{{ route('services.create') }}" class="btn btn-success mb-3 text-white">Add service</a>
                ...
                <table class="table table-centered table-nowrap mb-0 rounded">
                    ...
                    @foreach($services as $service)
                        ...
                        <tr>
                            ...
                            <td>{!! $service->desc['ru'] !!}</td>
                            <td>
                                <img src="{{ asset($service->img) }}" alt="" style="max-width: 250px">
                            </td>
                            <td>
                                <div class="actions d-flex flex-column">
                                    <form class="" action="{{ route('services.destroy', ['service' => $service->id]) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="text-danger bg-transparent border-0 p-0 m-0 mb-3 fw-bolder">delete</button>
                                    </form>
                                    <a href="{{ route('services.edit', ['service' => $service->id]) }}" class="text-info fw-bolder">edit</a>
                                </div>
                            </td>
                        </tr>
                        <!-- End of Item -->
